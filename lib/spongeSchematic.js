const { Vec3 } = require('vec3')
const versions = require('minecraft-data').versions.pc

function findVersion (dataVersion) {
  for (const v of versions) {
    if (v.dataVersion === dataVersion) {
      return v.minecraftVersion
    }
  }
  return versions[0].minecraftVersion // default to latest
}

function parseBlockName (str) {
  if (str.includes('[')) {
    let [name, prop] = str.split('[')
    prop = prop.substring(0, prop.length - 1)
    return { name: name.split(':')[1], properties: prop.split(',').map(x => x.split('=')) }
  } else {
    return { name: str.split(':')[1], properties: [] }
  }
}

function parseValue (value, state) {
  if (state.type === 'enum') {
    return state.values.indexOf(value)
  }
  if (value === 'true') return 0
  if (value === 'false') return 1
  return parseInt(value, 10)
}

function getStateValue (states, name, value) {
  let offset = 1
  for (let i = states.length - 1; i >= 0; i--) {
    const state = states[i]
    if (state.name === name) {
      return offset * parseValue(value, state)
    }
    offset *= state.num_values
  }
  return 0
}

function parsePalette (mcData, palette) {
  const out = []
  for (const [str, id] of Object.entries(palette)) {
    const { name, properties } = parseBlockName(str)

    const block = mcData.blocksByName[name]
    if (!block) {
      console.log(`Unknown block ${name} replacing with air`)
      out[id] = 0
      continue
    }
    let data = 0
    for (const [key, value] of properties) {
      data += getStateValue(block.states, key, value)
    }
    out[id] = block.minStateId + data
  }
  return out
}

function read (nbt, version) {
  const { Schematic } = require('../')
  if (!version) {
    version = findVersion(nbt.DataVersion)
  }
  const mcData = require('minecraft-data')(version)
  const palette = parsePalette(mcData, nbt.Palette)
  const size = new Vec3(nbt.Width, nbt.Height, nbt.Length)
  const offset = nbt.Metadata && nbt.Metadata.WEOffsetX
    ? new Vec3(nbt.Metadata.WEOffsetX, nbt.Metadata.WEOffsetY, nbt.Metadata.WEOffsetZ)
    : new Vec3(0, 0, 0)
  return new Schematic(version, size, offset, palette, nbt.BlockData)
}

function write (schematic) {
  // TODO
  throw new Error('Sponge write not implemented')
}

module.exports = { read, write }
